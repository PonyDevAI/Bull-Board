...
- [ ] **Build 通过**：与改动相关的构建成功：
  ...
  - 改 person（执行器）：`go build -o bb-person ./cmd/bb-person` 无报错。
...
- [ ] 对外命名仅使用 dashboard/console/**person**（执行器）；无 api/web/runner-go。见 docs/MASTER_PLAN_RUNNER_TO_PERSON_AND_CONSOLE.md。
