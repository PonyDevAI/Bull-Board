...
- `apps/control/package.json` - 仅 Fastify 依赖，移除 Prisma
- `apps/control/src/index.ts` - 仅注册 `GET /health`
- 删除：`apps/control/src/db.ts`、`apps/control/src/routes/*.ts`、`apps/control/prisma/schema.prisma`
...
1. **Control**：`curl -s http://localhost:3000/health` → `{"ok":true,"service":"bull-board-control"}`
